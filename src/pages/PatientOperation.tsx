...
const PatientOperation = () => {
  ...
  const toggleStrokeColor = (index: number, key: number) => {
    ...
    setSelectedTeeth((prevSelectedTeeth) => {
      // If key is already in the array, remove it; otherwise, add it
      const updatedTeeth = prevSelectedTeeth.includes(key)
        ? prevSelectedTeeth.filter((item) => item !== key)
        : [...prevSelectedTeeth, key];
  ...
  const onSubmit = (data: any) => {
    if (selectedteeth.length > 0) {
      const formData = {
        ...
        note: data?.note,
        total_cost: parseFloat(data?.prix),
  ...
  return (
    <Paper>
      ...
      <Box
        className={`w-full h-full flex flex-col md:flex-row justify-center items-center gap-4 p-4 `}
      >
        <Box
          className={`mx-auto my-auto w-[500px] max-w-full relative  ${
            clientage === "younger" ? "" : "hidden"
          }`}
        >
          <img
            src="/testmouth.png"
          ...
          >
            {[
              ...
              <polygon
                ...
                strokeWidth={3}
                fill="transparent"
                stroke={polygonColors[i]}
                onClick={() => toggleStrokeColor(i, p[1])}
                points={String(p[0])}
              />
            ))}
          </svg>
        ...
        >
          ...
          <Box className="  mt-4 mb-4 sm:mt-4 sm:mb-2 sm:ml-4 w-full flex flex-col gap-2 md:flex-row md:flex-wrap items-center  justify-center">
            <FormControl className="w-[80%] gap-3">
              <Box>
          ...
          <Box className=" flex p-4">
            <Button
              type="submit"
              variant="contained"
              className="w-full md:w-max !px-5 !py-3 rounded-lg !mx-auto "
            >
              Enregistrer
            </Button>
            <Button
